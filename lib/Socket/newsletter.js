...
export const makeNewsletterSocket = (config) => {
  ...
  return {
    ...
    // ================== TAMBAHAN BARU: INVITE ADMIN ==================
    newsletterAdminInvite: async (jid, userJid) => {
      return await executeWMexQuery(
        { newsletter_id: jid, user_id: userJid },
        QueryIds.ADMIN_INVITE_CREATE,
        XWAPaths.xwa2_newsletter_admin_invite_create
      );
    },
    newsletterAdminInviteRevoke: async (jid, userJid) => {
      return await executeWMexQuery(
        { newsletter_id: jid, user_id: userJid },
        QueryIds.ADMIN_INVITE_REVOKE,
        XWAPaths.xwa2_newsletter_admin_invite_revoke
      );
    },
    newsletterAdminInviteAccept: async (jid) => {
      return await executeWMexQuery(
        { newsletter_id: jid },
        QueryIds.ADMIN_INVITE_ACCEPT,
        XWAPaths.xwa2_newsletter_admin_invite_accept
      );
    },
    newsletterPendingInvites: async (jid) => {
      return await executeWMexQuery(
        { newsletter_id: jid },
        QueryIds.PENDING_INVITES,
        XWAPaths.xwa2_newsletter_pending_invites
      );
    }
    // ===================================================================
  };
